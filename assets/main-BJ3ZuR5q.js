(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))n(a);new MutationObserver(a=>{for(const o of a)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&n(r)}).observe(document,{childList:!0,subtree:!0});function s(a){const o={};return a.integrity&&(o.integrity=a.integrity),a.referrerPolicy&&(o.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?o.credentials="include":a.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(a){if(a.ep)return;a.ep=!0;const o=s(a);fetch(a.href,o)}})();function L(t=!1,e=""){return`
  ...
  `}function q(){const e=x({title:"Добавить новый контакт",content:`
    <form id="chatForm">
  ...
  `});document.querySelector(".app").insertAdjacentHTML("beforeend",e)}let u=d,m="";function w(){q();const t=document.getElementById("chatForm"),e=document.getElementById("avatarFile");t.addEventListener("submit",s=>k(s,e)),document.querySelector(".modal-close").addEventListener("click",v),document.querySelector(".modal-overlay").addEventListener("click",v)}function v(){const t=document.querySelector(".modal-overlay");t&&t.remove()}function k(t,e){t.preventDefault();const s=document.getElementById("name").value;if(s){const n=e.files[0],a=n?URL.createObjectURL(n):"public/defaultAvatar.png",o={id:String(d.length+1),avatar:a,title:s,message:"",time:new Date().toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"}),unreadCount:0,isRead:!0};d.push(o),u=d.filter(r=>r.title.toLowerCase().includes(m.toLowerCase())),b(),v()}}function A(t){const e=t.target.value.trim();if(e===m)return;m=e,u=d.filter(n=>n.title.toLowerCase().includes(m.toLowerCase()));const s=document.querySelector(".chatList");s.innerHTML="",u.forEach(n=>{const a=y(n);s.appendChild(a)})}function b(){const t=document.querySelector(".app");t.innerHTML="",t.insertAdjacentHTML("afterbegin",L());const e=document.getElementById("searchInput");e&&(e.value=m,e.addEventListener("input",A));const s=document.createElement("div");s.className="chatList",u.forEach(n=>{const a=y(n);s.appendChild(a)}),t.appendChild(s),t.appendChild(H(w))}const j=document.querySelector(".app"),h=()=>{const t=window.location.pathname;j.innerHTML="";const e=t.match(/\/chat\/(\d+)/);e?$(e[1]):b()},F=t=>{history.pushState(null,null,t),h()};document.body.addEventListener("click",t=>{const e=t.target.closest(".link");e&&F(e.getAttribute("data-link"))});window.addEventListener("popstate",h);h();
